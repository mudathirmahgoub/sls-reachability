...
"""
  ...
  (int.star-contains 
    ...
    # print(f"Parsed term: {slv.getValue(t)}")

    print(f"A.args: {A.args}")
    print(f"A.fmls: {A.fmls}")    
    print(f"B.args: {B.args}")
    print(f"B.fmls: {B.fmls}")

    # A(0) may be immediately satisfiable
    sls = semilinear.SLS(B, set_vars, len(B.args))
    sls.augment()
    print(f"sls.sets:{sls.sets}")
    print(f"sls.dim: {sls.dim}")
    print(f"sls.phi.args: {sls.phi.args}")   
    print(f"sls.phi.fmls: {sls.phi.fmls}")   
    print(f"sls.phi.set_vars: {sls.set_vars}")   
    print(f"sls.star: {sls.star()}")
    print(f"sls.getSLS: {sls.getSLS()}")    
