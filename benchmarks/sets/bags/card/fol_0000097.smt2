(set-logic ALL)


; forall b_ek:B. forall a_ej:A. forall a_ei:A. nonempty(b_ek & a_ej & a_ei & ~f)

(declare-fun n () Int)
(declare-fun t () Int)

(declare-fun f () (Bag Int))
; set semantics: f has no repeated element
(assert (= f (bag.setof f)))
(declare-fun UNIVERALSET () (Bag Int))
; set semantics: UNIVERALSET has no repeated element
(assert (= UNIVERALSET (bag.setof UNIVERALSET)))
(assert (bag.subbag f UNIVERALSET))
(assert (= (bag.card UNIVERALSET) n))

(assert (> n 0))
(assert (> n (* 3 t)))
(assert (<= (bag.card f) t))

(declare-fun b_ek () (Bag Int))
; set semantics: b_ek has no repeated element
(assert (= b_ek (bag.setof b_ek)))
(assert (bag.subbag b_ek UNIVERALSET))
(assert (>= (* 2 (bag.card b_ek)) (+ (+ n (* 3 t)) 1)))

(declare-fun a_ej () (Bag Int))
; set semantics: a_ej has no repeated element
(assert (= a_ej (bag.setof a_ej)))
(assert (bag.subbag a_ej UNIVERALSET))
(assert (>= (bag.card a_ej) (- n t)))

(declare-fun a_ei () (Bag Int))
; set semantics: a_ei has no repeated element
(assert (= a_ei (bag.setof a_ei)))
(assert (bag.subbag a_ei UNIVERALSET))
(assert (>= (bag.card a_ei) (- n t)))


(assert (= (bag.card (bag.inter_min (bag.inter_min (bag.inter_min b_ek a_ej) a_ei) (bag.difference_subtract UNIVERALSET f))) 0))

(check-sat)
