(set-logic ALL)


; forall c_fg:C. forall b_ff:B. forall b_fe:B. forall a_fd:A. nonempty(c_fg & b_ff & b_fe & a_fd & ~f)

(declare-fun n () Int)
(declare-fun t () Int)

(declare-fun f () (Bag Int))
; set semantics: f has no repeated element
(assert (= f (bag.setof f)))
(declare-fun UNIVERALSET () (Bag Int))
; set semantics: UNIVERALSET has no repeated element
(assert (= UNIVERALSET (bag.setof UNIVERALSET)))
(assert (bag.subbag f UNIVERALSET))
(assert (= (bag.card UNIVERALSET) n))

(assert (> n 0))
(assert (> n (* 3 t)))
(assert (<= (bag.card f) t))

(declare-fun c_fg () (Bag Int))
; set semantics: c_fg has no repeated element
(assert (= c_fg (bag.setof c_fg)))
(assert (bag.subbag c_fg UNIVERALSET))
(assert (>= (* 2 (bag.card c_fg)) (+ (- n t) 1)))

(declare-fun b_ff () (Bag Int))
; set semantics: b_ff has no repeated element
(assert (= b_ff (bag.setof b_ff)))
(assert (bag.subbag b_ff UNIVERALSET))
(assert (>= (* 2 (bag.card b_ff)) (+ (+ n (* 3 t)) 1)))

(declare-fun b_fe () (Bag Int))
; set semantics: b_fe has no repeated element
(assert (= b_fe (bag.setof b_fe)))
(assert (bag.subbag b_fe UNIVERALSET))
(assert (>= (* 2 (bag.card b_fe)) (+ (+ n (* 3 t)) 1)))

(declare-fun a_fd () (Bag Int))
; set semantics: a_fd has no repeated element
(assert (= a_fd (bag.setof a_fd)))
(assert (bag.subbag a_fd UNIVERALSET))
(assert (>= (bag.card a_fd) (- n t)))


(assert (= (bag.card (bag.inter_min (bag.inter_min (bag.inter_min (bag.inter_min c_fg b_ff) b_fe) a_fd) (bag.difference_subtract UNIVERALSET f))) 0))

(check-sat)
