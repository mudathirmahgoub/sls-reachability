...
; set semantics: c_cn has no repeated element
(assert (= c_cn (bag.setof c_cn)))
(assert (bag.subbag c_cn UNIVERALSET))
(assert (>= (* 2 (bag.card c_cn)) (+ (- n t) 1)))

(declare-fun c_cm () (Bag Int))
; set semantics: c_cm has no repeated element
(assert (= c_cm (bag.setof c_cm)))
(assert (bag.subbag c_cm UNIVERALSET))
(assert (>= (* 2 (bag.card c_cm)) (+ (- n t) 1)))

(declare-fun b_cl () (Bag Int))
; set semantics: b_cl has no repeated element
(assert (= b_cl (bag.setof b_cl)))
(assert (bag.subbag b_cl UNIVERALSET))
(assert (>= (* 2 (bag.card b_cl)) (+ (+ n (* 3 t)) 1)))


(assert (= (bag.card (bag.inter_min (bag.inter_min (bag.inter_min c_cn c_cm) b_cl) (bag.difference_subtract UNIVERALSET f))) 0))
